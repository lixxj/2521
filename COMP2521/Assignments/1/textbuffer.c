...
void formRichText(TB tb){
    ...
    while (currl != NULL) {
        ...
        if (currl->text[0] == '#') {
            currl->text[0] = '<';
            insert_substring(currl->text, "</h1>", len);
            insert_substring(currl->text, "h1>", 2);
        } else {  
            // "*" and "_"
            int from = 0;
            int to = 0;
            ...
            // "closing" situations
            while (to < len) {
                if (currl->text[to] == '*' && currl->text[to+1] != '*' 
                    && star == FALSE && line == FALSE) {
                    star = TRUE;      
                    from = to;
                
                } else if (currl->text[to] == '_' && currl->text[to+1] 
                           != '_' && line == FALSE && star == FALSE) {
                    line = TRUE;    
                    from = to;
                
                } else if (currl->text[to] == '*' && star == TRUE) {
                    currl->text[from] = '<';    
                    currl->text[to] = '<';
                    if (to == len - 2) {    
                        insert_substring(currl->text, "/b>", to +2);
                        insert_substring(currl->text, "b>", from + 2);
                    } else {
                        insert_substring(currl->text, "/b>", to + 2);
                        insert_substring(currl->text, "b>", from + 2);
                    }
                    star = FALSE;
                    len = strlen(currl->text);
                } else if (currl->text[to] == '_' && line == TRUE) {
                    currl->text[from] = '<';    
                    currl->text[to] = '<';
                    if (to == len - 2) {    
                        insert_substring(currl->text, "/i>", to +2);
                        insert_substring(currl->text, "i>", from + 2);
                    } else {
                        insert_substring(currl->text, "/i>", to + 2);
                        insert_substring(currl->text, "i>", from + 2);
                    }
            ...
            // "not closing" situations
            if (star == TRUE) { // * not closing
                star = FALSE;
                len = strlen(currl->text);
                to = from;
                while (to < len) {
                    if (currl->text[to] == '_' && currl->text[to+1] != 
                        '_' && line == FALSE && star == FALSE) {
                        line = TRUE;      
                        from = to;
            ...
            } else if (line == TRUE) { // _ not closing
                line = FALSE;
                len = strlen(currl->text);
                to = from;
                while (to < len) {   
