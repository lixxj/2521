...
// static void printGivenLevel(BSTree root, int level);
// static int height(BSTree node);
static int findMinimum(BSTree root);
...
// print values in level-order TODO
...
// BONUS ##BONUS1## TODO
// The following works but not using Queue Algorithm :(
// print values in level-order TODO
void BSTreeLevelOrder(BSTree t)
{
	int h = height(t); 
...
// Print nodes at a given level (template function)
static void printGivenLevel(BSTree root, int level) 
{ 
    if (root == NULL) {
        return; 
...
// down to the farthest leaf node. (template functio)
static int height(BSTree node) 
{ 
    if (node == NULL) {
        return 0; 
...
int countEven(BSTree t) 
{
    ...
    return countEven(t->left) + countEven(t->right) + (t->value % 2);
}    
...
// ##BONUS3## successor TODO
// Recursive function to find inorder successor for given key in a BST
int successor (BSTree t, int v) {
    // successor in right subtree
    if (t->value <= v) {
        if (t->right == NULL) {
            return NO;
        } else if (t->right->value == v) {
            return -2; // not able to handle this case 
        } else {
            return successor(t->right, v);       
        }
    } else { // successor either in left subtree or current node
        // no left subtree, then certainly current node
        if (t->left == NULL) {
            return t->value;
        } else if (t->left->value == v) {
            if (t->left->right != NULL) {
                return findMinimum(t->left->right);
            } else {
                return t->value;
            }
        } else {
...
// Paul's successor solution 
int succ(BSTree t, int v) {
    int res;
    if (t->value > v) {
        res = succ(t->left, v);
        // this line is the essence
        if (res == -1) res = t->value;
    } else if (t->value < v) {
        res = succ(t->right, v);
    } else {
        res = (t->right) ? findMinimum(t->right) : -1;
    }
    return res;
}
