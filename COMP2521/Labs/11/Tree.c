...
Link insertSplay(Link t, Item it)
{
	...
	if (diff == 0)
		...
		if (less(v,key(t->left->value)) && t->within->ncompares++) {
		...
		if (less(key(t->right->value),v) && t->within->ncompares++) {
...
static Link searchSplay(Link t, Key k, int *found)
{
	...
	else if (eq(key(t->value),k) && t->within->ncompares++) {
		*found = 1; // item found, store true  
		res =  t;  
	}
	else if (less(k,key(t->value)) && t->within->ncompares++) {
		if (t->left == NULL){
			*found = 0;// item not found
			//res = rotateRight(t); 
			res = t;
		}
		else if (eq(key(t->left->value),k) && t->within->ncompares++) {
			*found = 1;
			res = rotateR(t);
		}
		else {
			if (less(k,key(t->left->value)) && t->within->ncompares++) {
				// left-left
		...
		else if (eq(key(t->right->value),k) && t->within->ncompares++) {
			*found = 1;
			res = rotateL(t);
		}
		else{
			if (less(key(t->right->value),k) && t->within->ncompares++) {
				/* right-right */
