...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   ...
   while (curr != NULL) {
        ...
        if (curr->type == BOAT) {
            ...
            while (sea != NULL) {
                // check "end" accessible and is a city and hasn't been added 
                if (flag == 0 && sea->v == end && idToType(end) == LAND) {
                    type[ntypes] = sea->type;
                    ntypes++;
                    flag = 1;
                }
                sea = sea->next;
