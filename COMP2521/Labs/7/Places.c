...
int nameToID(char *name)
{
    ...
    */
    ...
    int first = MIN_MAP_LOCATION;           
    int last = MAX_MAP_LOCATION;     
    int middle;         
    while (first <= last) {
        middle = (first + last) / 2;
        if (strcmp(name,places[middle].name) == 0) {
            return places[middle].id;
        } else if (strcmp(name,places[middle].name) < 0) { // If name is in lower half
            last = middle - 1;
        } else { // in upper half
            first = middle + 1;    
        }       
